...
def build(user):
  ...
  <g clip-path="url(#scard)">
    ...
    a(f'      <circle class="core" cx="{pad+4}" cy="34" r="4" fill="{CYA}"/>')
    ...
    for i, (label, value, sub, col) in enumerate(tiles):
        ...
        a(f'      <rect class="tile" x="{x+14:.1f}" y="55" width="{tw-28:.1f}" height="2.5" rx="1.2" fill="{col}" opacity="0.3" style="animation-delay: {i*1.2}s"/>')
        a(f'      <g font-family="{MONO}">')
    ...
    for i in range(1, len(pts)):
        dist.append(dist[-1] + math.dist(pts[i - 1], pts[i]))
    span = dist[-1] or 1
    track = "\n".join(f"      {100 * v / span:.2f}% {{ transform: translate({pts[i][0]:.1f}px, {pts[i][1]:.1f}px); }}" for i, v in enumerate(dist))
    a(f'''  <style>
    .core {{ animation: core 1.8s ease-in-out infinite; }}
    @keyframes core {{ 0%, 100% {{ opacity: 1; }} 50% {{ opacity: 0.2; }} }}
    .tile {{ animation: tile 4.8s ease-in-out infinite; }}
    @keyframes tile {{ 0%, 100% {{ opacity: 0.3; }} 50% {{ opacity: 1; }} }}
    .seg {{ animation: seg 4.8s ease-in-out infinite; }}
    @keyframes seg {{ 0%, 100% {{ opacity: 0.9; }} 50% {{ opacity: 1; }} }}
    .draw {{ stroke-dasharray: 2600; animation: draw 2.6s ease-out 0.2s forwards; }}
    @keyframes draw {{ from {{ stroke-dashoffset: 2600; }} to {{ stroke-dashoffset: 0; }} }}
    .shimmer {{ animation: shimmer 4.5s linear infinite; }}
    @keyframes shimmer {{ from {{ transform: translateX(0); }} to {{ transform: translateX({W}px); }} }}
    .rider {{ animation: rider 5.5s linear infinite; }}
    @keyframes rider {{
{track}
    }}
  </style>''')
    ...
    a(f'    <path class="draw" d="{line}" fill="none" stroke="url(#sGrad)" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>')
    a(f'    <circle class="rider" r="4" fill="#FFFFFF"/>')
    ...
    for i, (name, size) in enumerate(top):
        ...
        a(f'      <rect class="seg" x="{cur:.1f}" y="{bar_y}" width="{seg:.1f}" height="{bar_h}" fill="{colors[name]}" opacity="0.9" style="animation-delay: {i * 0.5:.2f}s"/>')
        cur += seg
    a(f'    </g>')
    a(f'    <g clip-path="url(#sbar)"><rect class="shimmer" x="{pad-200}" y="{bar_y}" width="200" height="{bar_h}" fill="url(#shimmer)" opacity="0.55"/></g>')
